...
Fwv = function (entity)
    ...
    if car ~= 0 and (wasInCar or IsCar(car)) then
      ...
      if IsControlJustReleased(0, 47) and GetLastInputMethod(0) then
        beltOn = not beltOn 
        if beltOn then 
		  --exports['mythic_notify']:SendAlert('inform', 'Je hebt je gordel omgedaan!', 2000, { ['background-color'] = '#00ff00', ['color'] = '#ffffff' })
			exports['mythic_notify']:SendAlert('success', 'Je hebt je gordel omgedaan!')
		  SendNUIMessage({
		    displayWindow = 'false'
		    })
		  isUiOpen = true 
		else 
			--exports['mythic_notify']:SendAlert('Gelukt', 'Je hebt je gordel afgedaan!', 2000, { ['background-color'] = '#ff0000', ['color'] = '#ffffff' })
      exports['mythic_notify']:SendAlert('error', 'Je hebt je gordel Afgedaan!')
		  SendNUIMessage({
		     displayWindow = 'true'
		     })
		  isUiOpen = true  
