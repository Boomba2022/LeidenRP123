...
AddEventHandler('iens:repair', function()
	if isPedDrivingAVehicle() then
		...
		if IsNearMechanic() then
			...
			TriggerServerEvent('repareer:betaal', 500)
			notification("~g~De ANWB heeft je voertuig gemaakt! ~n~~y~Je hebt ~r~€1000~y~ betaald!")
			return
		end
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				notification("~g~" .. repairCfg.fixMessages[fixMessagePos] .. ", Ga snel naar de ANWB!")
				fixMessagePos = fixMessagePos + 1
				if fixMessagePos > repairCfg.fixMessageCount then fixMessagePos = 1 end
			else
				notification("~r~Je voertuig is zo beschadig, hij is niet meer te maken")
			end
		else
		...
		notification("~y~Je moet in een voertuig zitten om een voertuig te maken!")
	end
end)

RegisterNetEvent('iens:notAllowed')
AddEventHandler('iens:notAllowed', function()
	notification("~r~Je mag geen voertuigen maken! ~n~~s~Er is voldoende ~o~ANWB ~s~in de stad!")
end)
...
Citizen.CreateThread(function()
	TriggerEvent('chat:addSuggestion', '/repareer', 'Repareer je voertuig tijdelijk. Ga naast een garage staan om je voertuig volledig te repareren')
end)