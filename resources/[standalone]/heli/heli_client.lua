...
local helicam = false -- is in helicam
local aw139_hash = GetHashKey("aw139") -- change to another heli if you want :P
local fov = (fov_max + fov_min) * 0.5
local vision_state = 0 -- 0 is normal, 1 is night vision, 2 is thermal vision
local spotlight_state = false
...
function IsPlayerInAw139()
    local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1))
    return IsVehicleModel(vehicle, aw139_hash)
end
