...
Citizen.CreateThread(function()
	while true do
		...
		if DoesEntityExist(closestObject) then
			...
			sleep = 5
				DrawText3Ds(sitCoords, "[E] ile Otur", 0.4)
			...
			sleep = 5
				DrawText3Ds(pickupCoords, "[E] ile Tut, [X] ile Bırak ", 0.4)
				if IsControlJustPressed(0, 38) then
...
Sit = function(wheelchairObject)
	...
	if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
		if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'missfinale_c2leadinoutfin_c_int', '_leadin_loop2_lester', 3) then
			ShowNotification("Sandalye zaten kullanılıyor!")
			return
		end
	end
