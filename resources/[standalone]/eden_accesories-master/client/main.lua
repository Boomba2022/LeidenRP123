...
RegisterNetEvent('eden_accesories:demper')
AddEventHandler('eden_accesories:demper', function(duration)
	local inventory = ESX.GetPlayerData().inventory
	local demper = 0

		for i=1, #inventory, 1 do
		  if inventory[i].name == 'demper' then
			demper = inventory[i].count
		  end
		end
...
local currentWeaponHash = GetSelectedPedWeapon(ped)
		...
		if used < demper then
			if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
		  		 ...
		  		 ESX.ShowNotification(("Je hebt jezelf zojuist uitgerust met een geluiddemper. Het zal elke keer dat je terugkeert naar de stad opnieuw moeten worden uitgerust.")) 
		  		 	used = used + 1
	...
	used = used + 1
		  		  ...
		  		  ESX.ShowNotification(("Je hebt geen wapen in je handen of je wapen is niet geschikt voor de demper."))	
			end
			else
					  		 ESX.ShowNotification(("Je hebt al je geluiddempers gebruikt.")) 
		end
end)
				local used2 = 0
