...
AddEventHandler('playerDropped', function (reason) 
	...
	if PermTracker[discord] ~= nil then 
		...
		for i = 1, #list do 
			...
			print("[LeidenRP] (playerDropped) Removed " 
	...
	local discord = ExtractIdentifiers(src).discord:gsub("discord:", "");
		...
		if identifierDiscord then
				...
				if not (roleIDs == false) then
					...
					for i = 1, #roleList do
						for j = 1, #roleIDs do
							if exports.Badger_Discord_API:CheckEqual(roleList[i][1], roleIDs[j]) then
								print("[LeidenRP] (playerConnecting) Added " .. GetPlayerName(src) .. " to role group " .. roleList[i][2]);
					...
					print("[LeidenRP] (playerConnecting) Player " .. GetPlayerName(src) .. " has been granted their permissions...");
				else
					--print("[LeidenRP] " .. GetPlayerName(src) .. " has not gotten permissions because we could not find their roles...")
					if InDiscordDetector[license] == nil then 
						-- Notify them they are not in the Discord 
						InDiscordDetector[license] = true;
						deferrals.done('[LeidenRP] Je zit nog niet in onze discord...' .. 
							' Zo kunnen we jou rollen niet geven ' ..
							'relog AUB.');
						return;
					end
				end
			...
			if DiscordDetector[license] == nil then 
				...
				--print('[LeidenRP] Discord was not found for player ' .. GetPlayerName(src) .. "...")
				deferrals.done('[LeidenRP] Discord niet gevonden... Join onze discord om toegang te kunnen krijgen ' ..
					'https://discord.LeidenRP.nl')
				return;
			end
		end
	deferrals.done();
