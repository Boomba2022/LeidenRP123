	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_actions', {
		...
		if data.current.value == 'vehicle_list' then
			if Config.EnableSocietyOwnedVehicles then
				...
				local elements = {
					{label = ('Mercedes vrachtwagen(Oprij)'),  value = 'anwbmercedes'},
					{label = ('Mercedes vrachtwagen(Kabel)'),  value = 'flatbed'},
					{label = ('Mercedes vito'),  value = 'VITO'},
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:OpenBossMenu1', 'mechanic', function(data, menu)
				menu.close()
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, 121) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
			if NPCOnJob then
