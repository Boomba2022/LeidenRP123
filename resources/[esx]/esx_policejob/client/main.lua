...
function setUniform(uniform, playerPed)
	TriggerEvent('skinchanger:getSkin', function(skin)
		...
		if uniformObject then
			...
			elseif uniform == 'dsi_wear' then
				SetPedArmour(playerPed, 100)
			elseif uniform == 'at_wear' then
				SetPedArmour(playerPed, 100)
			elseif uniform == 'honden_wear' then
				SetPedArmour(playerPed, 100)
			end
		else
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{label = _U('steek_wear'), uniform = 'steek_wear'},
		{label = _U('helm_wear'), uniform = 'helm_wear'},
		{label = _U('motor_wear'), uniform = 'motor_wear'},
		{label = _U('motors_wear'), uniform = 'motors_wear'},
		{label = _U('gilet_wear'), uniform = 'gilet_wear'},
		{label = _U('honden_wear'), uniform = 'honden_wear'},
		{label = _U('ibt_wear'), uniform = 'ibt_wear'},
		{label = _U('dsi_wear'), uniform = 'dsi_wear'},
		{label = _U('at_wear'), uniform = 'at_wear'},
				...
				if not isInService then
					...
					ESX.TriggerServerCallback('esx_service:enableService', function(canTakeService, maxInService, inServiceCount)
						if not canTakeService then
							...
							exports["rp-radio"]:GivePlayerAccesToFrequencies(1, 2, 3, 4, 5, 6, 7, 8)
