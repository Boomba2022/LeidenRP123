...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            ...
            exports['mythic-notify']:SendAlert('error', 'Je bent al met iets bezig!')
        end
    else
        exports['mythic-notify']:SendAlert('error', 'Kan actie niet uivoeren!')
    end
end
...
function DisableActions(ped)
    ...
    if Action.controlDisables.disableanim then
        DisablePlayerFiring(PlayerId(), true) -- Disable weapon firing
