        ...
        Citizen.Wait(0)
			...
			for k in pairs(locations) do
				if GetDistanceBetweenCoords(locations[k].x, locations[k].y, locations[k].z, GetEntityCoords(GetPlayerPed(-1))) < 150 then			
					...
					if GetDistanceBetweenCoords(locations[k].x, locations[k].y, locations[k].z, GetEntityCoords(GetPlayerPed(-1)), false) < 2 then			
						TriggerServerEvent('KWiet:get')
						TriggerEvent('KWiet:new', k)
					end
			...
			if GetDistanceBetweenCoords(Config.Processing.x, Config.Processing.y, Config.Processing.z, GetEntityCoords(GetPlayerPed(-1))) < 150 then			
					...
					if GetDistanceBetweenCoords(Config.Processing.x, Config.Processing.y, Config.Processing.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then			
						Draw3DText( Config.Processing.x, Config.Processing.y, Config.Processing.z , "~w~Wiet Productie~y~\nDruk op [~b~E~y~] om het process te starten",4,0.15,0.1)
...
local making = true
	...
	TriggerEvent('esx:showNotification', '~w~Wiet ~g~productie ~g~is ~g~gestart')
	Citizen.Wait(5000)
	
	ESX.TriggerServerCallback('KWiet:process', function(output)
			making = output
		end)
...
RegisterNetEvent('KWiet:start')
AddEventHandler('KWiet:start', function()
	local set = false
	Citizen.Wait(10)
...
RegisterNetEvent('KWiet:new')
AddEventHandler('KWiet:new', function(id)
	local set = false
	Citizen.Wait(10)
...
RegisterNetEvent('KWiet:message')
AddEventHandler('KWiet:message', function(message)
