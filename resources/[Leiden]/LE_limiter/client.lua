	...
	while true do 
		...
		local float Max = GetVehicleMaxSpeed(vehicleModel)
			if ( ped ) then
				...
				SendNUIMessage({
						cruise = true,
						TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Limiter aangezet!' })
					})else
					SetEntityMaxSpeed(vehicle, Max)
					enableCruise = false
					TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = 'Limiter uitgezet!' })
					SendNUIMessage({
						cruise = false,
					})
					end 
						else
							TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = 'Kan niet' })
						end
					end
				end
			end
